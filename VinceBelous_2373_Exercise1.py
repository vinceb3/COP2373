# Establish global constants. These values will be manipulated by functions, but
# the values of these variables themselves will stay the same.
TOTAL_TICKETS = 20
MAX_PER_BUYER = 4

# The sell_loop() function contains the core of the program, a while loop that
# engages another function to ask the user how many tickets they want and then
# registers that data, applying it to accumulators. It accepts two arguments
# that correspond to the two global constants.
def sell_loop(total_tickets, max_per_buyer):
    ...
    while tickets_sold < total_tickets:
        # Calls the user_tickets() function, which returns the number of desired
        # tickets to the tickets_user_bought variable.
...
# The user_tickets() function prompts the user for how many tickets they want.
# Once a valid number is entered, that value is returned. This function accepts
# two arguments, one corresponding to the MAX_PER_BUYER global constant, and the
# other the number of tickets left.
def user_tickets(tickets_available, max_per_buyer):
    ...
    # Tests the user's value for validity. It cannot be greater than the
    # number of tickets available or maximum tickets per buyer.
